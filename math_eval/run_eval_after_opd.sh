...
# Default: AIME24/25 + AMC23 (n=8) + MATH-500 (n=4), which is the standard
# math-eval suite used for thinking-mode checkpoints in this project.
# To include others (hmmt24/25, olympiadbench) pass them explicitly via
# BENCHMARKS, e.g.:
#     BENCHMARKS="aime24,olympiadbench" bash run_eval_after_opd.sh
declare -a DATASETS=(
    "aime24"
    "aime25"
    "amc23"
...
declare -a INPUTS=(
    ...
    "${GOPD_ROOT}/data/MATH-500/test.jsonl"
)
# Per-dataset k in pass@k. AIME/AMC (~30-40 problems) use n=8;
# MATH-500 (500 problems) uses n=4 which is enough for a stable pass@k.
declare -a N_LIST=(
    16    # aime24
    16   # aime25
    16   # amc23
...
# Optional: override the per-dataset sample count for ALL datasets via env.
# Useful when we just want a quick signal from a checkpoint (e.g. SFT eval
# with N_OVERRIDE=4). Applied AFTER the subset filter below.
if [[ -n "${N_OVERRIDE:-}" ]]; then
    if ! [[ "${N_OVERRIDE}" =~ ^[0-9]+$ ]] || [[ "${N_OVERRIDE}" -le 0 ]]; then
        echo "[fatal] N_OVERRIDE must be a positive integer, got: '${N_OVERRIDE}'"
        exit 3
    fi
    for _i in "${!N_LIST[@]}"; do
        N_LIST[$_i]="${N_OVERRIDE}"
    done
    echo "[info] N_OVERRIDE=${N_OVERRIDE} -> forcing n=${N_OVERRIDE} for every dataset"
fi
...
TOP_K=${TOP_K:-20}
SEED=${SEED:-42}
# ENABLE_THINKING=1 -> pass --enable_thinking to eval_math.py so Qwen3-style
# chat template opens a <think>...</think> block. Defaults to OFF for
...
# =====================================================================
# Optional --enable_thinking flag for eval_math.py. We build the flag as
# an array so it either expands to nothing (thinking OFF) or a single arg
# (thinking ON), rather than trying to sneak an empty string past argparse.
# =====================================================================
declare -a THINKING_FLAG=()
if [[ "${ENABLE_THINKING}" == "1" || "${ENABLE_THINKING,,}" == "true" || "${ENABLE_THINKING,,}" == "yes" ]]; then
    THINKING_FLAG=(--enable_thinking)
    echo "[info] enable_thinking=ON -> passing --enable_thinking to eval_math.py"
fi
